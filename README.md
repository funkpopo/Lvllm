...
| `LVLLM_GPU_PREFILL_MIN_BATCH_SIZE` | GPU Prefill Parameter | `0` | Minimum input length for using GPU prefill `4096`: when input length reaches this value, start GPU prefill | The value should not be too small, set to 0 to disable GPU prefill function |
| `LVLLM_GPU_PREFILL_CONFIG_HARD_CHECK` | GPU Prefill Parameter | `1` | Whether to fail fast for invalid startup combinations (for example, `LVLLM_GPU_PREFILL_MIN_BATCH_SIZE > max_num_batched_tokens`) | Set to `0` to downgrade invalid combinations to warning logs |
| `LVLLM_GPU_PREFILL_AUTO_CLAMP` | GPU Prefill Parameter | `0` | Whether to clamp out-of-range startup values automatically | `1` clamps `LVLLM_GPU_PREFILL_MIN_BATCH_SIZE` down to `max_num_batched_tokens` |
